...
class VideoController < ApplicationController
  def list
    list_id = params[:id] || VideoList.pluck(:id).min
    ...
    if list_id.nil?|| list_id.to_i <= 0
      ...
      @related_keywords = RelatedKeyword.where(:keyword_string => @video_list_name).first
  ...
  def show
    ...
    @related_keywords = RelatedKeyword.where(:keyword_string => @video.name).first
    unless @related_keywords.nil?
      @related_keywords_list = RelatedKeyword.where(:keyword_string => @related_keywords.related_keywords.split(","))
    end
  end
